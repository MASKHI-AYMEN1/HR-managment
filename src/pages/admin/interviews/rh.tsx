...
export default function InterviewRHPage() {
    const intl = useIntl()
    ...
    const { data: paged, isLoading } = useListCandidaturesForRH(
        page,
        PAGE_SIZE,
        search || undefined,
        isClosed,
    ...
    const openEvaluation = (candidatureId: string) => {
        router.push(`/admin/interviews/rh-evaluate?candidatureId=${candidatureId}`)
    }
            ...
            case 'actions':
                return (
                    <Button size="sm" color="warning" onClick={() => openEvaluation(item.candidatureId)}>
                        {item.evaluationRH && item.evaluationRH.isClosed ? 'Modifier' : 'Évaluer'}
                    </Button>
                )
            default:
                return null
    ...
    const candidaturesWithId = candidatures.map(c => ({
        ...c,
        id: c.candidatureId, // Add id property using candidatureId
    }))
