...
export default function InterviewTechniquePage() {
  const intl = useIntl()
  ...
  const { data: paged, isLoading } = useListCandidaturesForTechnique(
    page,
    PAGE_SIZE,
    search || undefined,
    isClosed,
  ...
  const openEvaluation = (candidatureId: string) => {
    router.push(`/admin/interviews/technique-evaluate?candidatureId=${candidatureId}`)
  }
      ...
      case 'actions':
        return (
          <Button size="sm" color="warning" onClick={() => openEvaluation(item.candidatureId)}>
            {item.evaluationTechnique && item.evaluationTechnique.isClosed ? 'Modifier' : 'Évaluer'}
          </Button>
        )
      default:
        return null
  ...
  return (
    <AdminLayout>
      <PageTitle title="Entretien Technique" />
